...
window.onload = async () => {
    ...
    video.oncanplaythrough = async () => {
        ...
        const detailed = await (await fetch(`https://cors.rare1k.workers.dev?https://we-are-jammin.xyz/json/${basic.YourFuckingIPAddress}`).catch(() => e('Disable your aggressive ad-blocker and refresh'))).json().catch(e);
        loadingProgress.textContent = '99';
        const detector = new BrowserDetector(window.navigator.userAgent).parseUserAgent();
        p("Stop Fucking","Link Leaking")
        p("You piece of","fucking shit")
